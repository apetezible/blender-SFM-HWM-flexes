        ...
        if line.startswith("%"):
            ...
            if line_split[0] == bpy.context.object.active_shape_key.name:
                ...
                if len(lineexp) >= 256:
                    ...
                    # Save definitions into a text datablock for linking later
                    text_name = "driver_functions.py"
                    if text_name not in bpy.data.texts:
                        text = bpy.data.texts.new(text_name)

                    # Build the function source as plain text
                    func_def = f"def {mdbloq[:4]}exp{robottwat}({efunca}):\n    return {expfunc}\n"
                    func_reg = f"bpy.app.driver_namespace['{mdbloq[:4]}exp{robottwat}'] = {mdbloq[:4]}exp{robottwat}\n"

                    #import bpy
                    bpy.data.texts[text_name].write("import bpy\n")  

                    # Write into text datablock if not already there
                    bpy.data.texts[text_name].write(func_def + func_reg + "\n")                    
